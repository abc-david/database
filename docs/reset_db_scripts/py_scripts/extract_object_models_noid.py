#!/usr/bin/env python3
"""
Script to extract all object_models from SQL dump file and create INSERT statements without IDs
"""
...
OUTPUT_FILE = "services/database/docs/object_models_inserts_noid.sql"

def extract_object_models():
    """Extract all object_models records from the SQL dump file and create INSERTs without IDs"""
    ...
    # Generate INSERT statements without IDs
    with open(OUTPUT_FILE, 'w') as f:
        f.write("-- Generated INSERT statements for object_models without IDs\n\n")
        ...
        for row in rows:
            ...
            # Create INSERT statement without ID
            insert = f"""INSERT INTO public.object_models (name, object_type, version, definition, description, use_cases, related_templates, created_at, updated_at)
VALUES (
  '{fields[1].replace("'", "''")}',
  '{fields[2]}',
  '{fields[3]}',
